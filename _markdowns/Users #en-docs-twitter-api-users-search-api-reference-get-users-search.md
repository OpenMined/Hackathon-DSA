...
::: is-table-default
The users endpoint returns Users that match a search query. This
endpoint has a limit of 10,000 requests per 24 hours (in addition to the
...
+-----------------------+-----------------------+-----------------------+
| ` expansions `\       | enum (                | [Ex                   |
| [Optional]{.small}    | ` pinned_tweet_id ` ) | pansions](/en/docs/tw |
|                       |                       | itter-api/expansions) |
...
|                       | in_reply_to_user_id ` | returned pinned       |
|                       | , ` lang ` ,          | Tweet. Specify the    |
|                       | `                     | desired fields in a   |
|                       |  non_public_metrics ` | comma-separated list  |
|                       | , ` public_metrics `  | without spaces        |
|                       | , ` organic_metrics ` | between commas and    |
|                       | ,                     | fields. The Tweet     |
|                       | ` promoted_metrics `  | fields will only      |
|                       | ,                     | return if the user    |
|                       | `                     | has a pinned Tweet    |
|                       |  possibly_sensitive ` | and if you\'ve also   |
|                       | ,                     | included the          |
|                       | ` referenced_tweets ` | ` expansi             |
|                       | , ` reply_settings `  | ons=pinned_tweet_id ` |
|                       | , ` source ` ,        | query parameter in    |
|                       | ` text ` ,            | your request. While   |
|                       | ` withheld ` )        | the referenced Tweet  |
|                       |                       | ID will be located in |
|                       |                       | the original Tweet    |
...
|                       | , ` protected ` ,     | will deliver with     |
|                       | ` public_metrics ` ,  | each returned users   |
|                       | ` url ` ,             | objects. Specify the  |
|                       | ` username ` ,        | desired fields in a   |
|                       | ` verified ` ,        | comma-separated list  |
|                       | ` verified_type ` ,   | without spaces        |
|                       | ` withheld ` )        | between commas and    |
|                       |                       | fields. These         |
|                       |                       | specified user fields |
